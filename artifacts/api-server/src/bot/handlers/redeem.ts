...
import { redeemPanel, successPanel, alertPanel } from "../utils/format.js";
...
export async function showRedeemConfirm(ctx: BotContext): Promise<void> {
  ...
  if (stockCount === 0) {
    const text = alertPanel("OUT OF STOCK", [
  ...
  const text = redeemPanel(REDEEM_COST, user.balance, stockCount);
  const keyboard =
...
export async function handleRedeemConfirm(ctx: BotContext): Promise<void> {
  ...
  if (msgId) {
    await editAnimated(ctx, msgId, REDEEM_FRAMES, 400);
  }
  ...
  await db
    .update(usersTable)
    .set({ balance: newBalance, totalRedeems: user.totalRedeems + 1 })
    .where(eq(usersTable.id, user.id));

  // Forward the actual .txt file
  try {
  ...
  const text = successPanel("REDEEM SUCCESS", [
